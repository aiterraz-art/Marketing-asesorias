...
begin
  if not exists (select 1 from public.campaigns limit 1) then
    ...
    select id, 'Reel: 3 Errores Sentadilla', 'reel', 'scripted', now() + interval '2 days'
    from public.campaigns where title = 'Lanzamiento Reto Verano';

    insert into public.content_items (campaign_id, title, type, status, scheduled_date)
    select id, 'Post: Beneficios Creatina', 'post', 'idea', now() + interval '4 days'
    from public.campaigns where title = 'Lanzamiento Reto Verano';
  end if;
end $$;
...
create policy "Enable all access for all users" on public.student_measurements
for all using (true) with check (true);
...
declare
  ...
  carb_grams numeric;
begin
  -- Mifflin-St Jeor Equation (Male version as default, can be extended)
  ...
  if carb_cals < 0 then carb_cals := 0; end if;
  ...
  return json_build_object(
    ...
    'protein_g', p_protein_grams,
    'fat_g', p_fat_grams,
    'carbs_g', round(carb_grams, 0)
  );
end;
$$ language plpgsql;
...
create policy "Enable all access for all users" on public.student_plans
for all using (true) with check (true);

-- Insert Mock Students
insert into public.students (full_name, age, height, weight, activity_level, goal)
values 
('Juan Pérez', 28, 175, 82, 1.55, 'cut'),
('María García', 32, 162, 65, 1.375, 'maintenance'),
('Carlos Ruiz', 24, 180, 75, 1.725, 'bulk')

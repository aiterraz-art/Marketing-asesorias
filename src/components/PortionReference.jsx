...
const PortionReference = ({ forceOpen = false, hideHeader = false }) => {
    const [foods, setFoods] = useState([]);
    const [isOpen, setIsOpen] = useState(false);
    const [loading, setLoading] = useState(true);

    // If forceOpen or hideHeader is true, we show content.
    useEffect(() => {
        if (forceOpen || hideHeader) setIsOpen(true);
    }, [forceOpen, hideHeader]);
    ...
    const showContent = isOpen || forceOpen || hideHeader;
    ...
    return (
        <div className={hideHeader ? '' : 'mb-8 rounded-2xl bg-zinc-900 border border-zinc-800 overflow-hidden'}>
            {!hideHeader && (
                <button
            ...
            {showContent && (
                <div className={hideHeader ? '' : 'p-4 pt-0'}>
